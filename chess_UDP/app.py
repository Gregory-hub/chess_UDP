...
class App:
        ...
        while self.game_is_played:
            if self.chessboard.turn != self.player_color and self.udp_client.message_read == False:
                ...
                while not ok_move:
                    ...
                    if not self.chessboard.is_legal(chess.Move.from_uci(input_str)):
                        print("Illegal move. Try again")
                        ok_move = False

                self.udp_client.send_to_opponent(input_str)
                self.chessboard.push_uci(input_str)
                print(f"\Your move: {input_str}")
                print(self.chessboard)
    ...
    #     # stalls the program
    #     text = f"[App is running] at {self.udp_client.ip}:{self.udp_client.port}\n"
    ...
    #             case "/create":
    #                 self.udp_client.wait_for_connection()
